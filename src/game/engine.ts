...
import type { Aggression, Asset, BehaviorInterval, CombatEvent, CombatExchange, CombatSequence, ContactInterval, ContactObservation, DefenseCue, DetectionWindow, DoctrineLesson, EnemyFlight, FlightMode, IntelLevel, IntelReport, InterceptPlan, MatchState, Point, RadarTrackReceipt, ReinforcementCall, ReinforcementType, Role, RoundResult, Squadron, UnitFrame, WeaponEffect, WeaponKind } from './types'
...
const FIGHTER_RECON_ATTACK_RANGE=3.2
...
function fighterPursuit(round:number,index:number,fighter:SimUnit,recon:SimUnit,location:Point,start:number,seed:number){
  let roll=seed;const exchanges:CombatExchange[]=[];const effects:WeaponEffect[]=[]
  for(let shot=0;shot<4&&recon.aircraft>0;shot++){
    const time=start+.35+shot*.5;const kind:WeaponKind=shot%2===0?'air-to-air-missile':'gun';const hit=rand(roll++)<.68;const amount=hit?(kind==='gun'?Math.round(12+rand(roll++)*7):Math.round(26+rand(roll++)*12)):0
    const applied=hit?damage(recon,amount):{before:recon.aircraft,after:recon.aircraft,strength:recon.strength,morale:recon.morale}
    exchanges.push({id:`pursuit-${round}-${index}-${shot}`,time,attackerId:fighter.id,defenderId:recon.id,weapon:kind,damage:amount,moraleDamage:hit?8:0,hit,position:location,targetStrength:applied.strength,targetMorale:applied.morale})
    effects.push(weapon(`weapon-pursuit-${round}-${index}-${shot}`,kind,fighter.id,recon.id,time-.16,[location[0]-.55,location[1]+.2],[location[0]+.5,location[1]-.2],hit,amount))
  }
  return {exchanges,effects,roll}
}

function simulateRound(state:MatchState):RoundResult{
  const base=playerBase(state); const assets=structuredClone(state.enemyAssets); const playerAssets=structuredClone(state.playerAssets); const squadrons=structuredClone(state.squadrons); const flights=enemyFlights();
  ...
  for(let time=TICK_SECONDS;time<=60+1e-6;time+=TICK_SECONDS){
    ...
    for(const u of units){if(terminal(u)||u.mode==='dogfighting'||u.mode==='attacking-recon')continue;const home=u.friendly?base:ENEMY_BASE;let destination:Point=u.position
    ...
    // 3: contacts and actual communications.
    const current=new Map<string,{target:SimUnit;source:'radar'|'visual'}[]>(); const radar=playerAssets.find(a=>a.kind==='radar'&&a.health>0)
    for(const friendly of units.filter(u=>u.friendly&&!terminal(u))){const seen:{target:SimUnit;source:'radar'|'visual'}[]=[];for(const enemy of units.filter(u=>!u.friendly&&!terminal(u))){const visual=distance(friendly.position,enemy.position)<=effectiveSensorRange(friendly.source as Squadron);contact(friendly.id,enemy.id,'visual',enemy.position,visual,time);if(visual)seen.push({target:enemy,source:'visual'});const linked=!!radar&&distance(radar.position,enemy.position)<=RADAR_RANGE&&distance(friendly.position,radar.position)<=RADAR_COMMUNICATION_RANGE;contact(friendly.id,enemy.id,'radar',enemy.position,linked,time);const key=`${friendly.id}:${enemy.id}`;const receipt=openReceipts.get(key);if(linked){if(receipt)receipt.end=time;else{const next={id:`receipt-${receipts.length}`,radarId:radar!.id,receiverId:friendly.id,targetId:enemy.id,start:time,end:time};receipts.push(next);openReceipts.set(key,next)}seen.push({target:enemy,source:'radar'})}else if(receipt)openReceipts.delete(key)}current.set(friendly.id,seen)}
    // 4-5 doctrine, pursuit, merge.
    for(const u of units.filter(u=>u.friendly&&!terminal(u)&&u.mode!=='dogfighting'&&u.mode!=='attacking-recon'&&u.mode!=='recovering')){const seen=current.get(u.id)??[];const candidates=seen.filter(x=>u.role==='fighter'&&(x.target.role==='fighter'?(u.aggression==='aggressive'||x.source==='visual'):u.aggression==='aggressive')).sort((a,b)=>(a.target.role==='fighter'?0:1)-(b.target.role==='fighter'?0:1)||distance(u.position,a.target.position)-distance(u.position,b.target.position));const choice=candidates[0]
      ...
      if(u.role==='fighter'&&u.aggression==='conservative'&&seen.length){transition(u,'recovering',time,'conservative-contact');add('info','FIGHTER AVOIDS CONTACT',`${u.callsign} recovers under CONSERVATIVE doctrine.`,u.position,time);continue}
      if(choice){const total=u.traveled+distance(u.position,choice.target.position)+distance(choice.target.position,base);if(total<=MAX_FLIGHT_DISTANCE.fighter){if(u.mode!=='intercepting'||u.targetId!==choice.target.id){transition(u,'intercepting',time,choice.source==='visual'?'visual-contact':'radar-receipt',choice.target.id,choice.source);plans.push({squadronId:u.id,targetId:choice.target.id,start:time,end:time,source:choice.source,outcome:'merge'});add('friendly','FIGHTER INTERCEPT',`${u.callsign} diverts on a ${choice.source.toUpperCase()} track.`,u.position,time)}u.lastKnown=[...choice.target.position] as Point}else transition(u,'recovering',time,'fuel-reserve')}else if(u.mode==='intercepting'){transition(u,'pursuing-last-known',time,'track-lost',u.targetId);add('warning','FIGHTER TRACK LOST',`${u.callsign} continues to the last known position.`,u.lastKnown,time)}
    }
    if(!activeDogfight&&!activeReconPursuit)for(const a of units.filter(u=>u.friendly&&u.role==='fighter'&&u.mode==='intercepting')){const b=a.targetId?byId.get(a.targetId):undefined;const engagementRange=b?.role==='recon'?FIGHTER_RECON_ATTACK_RANGE:FIGHTER_MERGE_RANGE;if(!b||terminal(b)||distance(a.position,b.position)>engagementRange)continue;const location:[number,number]=[(a.position[0]+b.position[0])/2,(a.position[1]+b.position[1])/2]
      if(b.role==='fighter'){activeDogfight={a,b,location,start:time,exchanges:[],next:time};transition(a,'dogfighting',time,'merge',b.id,'visual');transition(b,'dogfighting',time,'merge',a.id,'visual');add('danger','DOGFIGHT COMMITS',`${a.callsign} merges with ${b.callsign}.`,location,time);break}
      if(b.role==='recon'&&a.aggression==='aggressive'){
        const result=fighterPursuit(state.round,sequences.length,a,b,location,time,roll);const end=time+.35+result.exchanges.length*.5;roll=result.roll;transition(a,'attacking-recon',time,'recon-intercept',b.id,'visual');a.position=[...location];b.targetId=undefined
        if(b.aircraft<=0){transition(b,'destroyed',time+.35+result.exchanges.length*.5,'fighter-pursuit');add('friendly','RECON DESTROYED',`${a.callsign} destroys ${b.callsign} in an air-to-air pursuit.`,location,time+result.exchanges.at(-1)!.time-time)}else{transition(b,'following-route',time,'fighter-pursuit');add('friendly','RECON UNDER PURSUIT',`${a.callsign} attacks ${b.callsign}; the recon flight survives and continues its route.`,location,time)}
        sequences.push({id:`pursuit-${sequences.length}`,kind:'pursuit',participantIds:[a.id,b.id],location,start:time,end,exchanges:result.exchanges,finalDisposition:{[a.id]:'rtb',[b.id]:b.mode==='destroyed'?'destroyed':'enroute'}});effects.push(...result.effects);activeReconPursuit={fighter:a,end};break
      }
    }
    ...
    if(activeDogfight&&time>=activeDogfight.next){const d=activeDogfight;roll+=1;const attacker=activeDogfight.exchanges.length%2?d.b:d.a;const defender=attacker===d.a?d.b:d.a;const hit=rand(roll)>.31;const damage=hit?22:0;defender.strength=Math.max(0,defender.strength-damage);defender.morale=Math.max(0,defender.morale-damage*.6);d.exchanges.push({id:`fight-${d.exchanges.length}`,time,attackerId:attacker.id,defenderId:defender.id,weapon:'gun',damage,moraleDamage:damage*.6,hit,position:d.location,targetStrength:defender.strength,targetMorale:defender.morale});d.next=time+.35;if(defender.strength<=0||d.exchanges.length>=8){if(defender.strength<=0){defender.aircraft=0;transition(defender,'destroyed',time,'dogfight-loss')}transition(attacker,'recovering',time,'dogfight-complete');sequences.push({id:`dogfight-${sequences.length}`,kind:'dogfight',participantIds:[d.a.id,d.b.id],location:d.location,start:d.start,end:time,exchanges:d.exchanges,finalDisposition:{[d.a.id]:d.a.mode==='destroyed'?'destroyed':'rtb',[d.b.id]:d.b.mode==='destroyed'?'destroyed':'rtb'}});add('friendly','DOGFIGHT ENDS',`${attacker.callsign} survives the engagement and recovers.`,d.location,time);activeDogfight=undefined}}
    if(activeReconPursuit&&time>=activeReconPursuit.end){transition(activeReconPursuit.fighter,'recovering',time,'recon-pursuit-complete');activeReconPursuit=undefined}
    ...
    if(time>=22&&units.every(terminal)&&!activeDogfight&&!activeReconPursuit)break
  }
  ...
  const executionRoutes=Object.fromEntries(units.map(u=>[u.id,u.frames.map(f=>f.position)]));const result:RoundResult={duration,tickSeconds:TICK_SECONDS,unitTracks:units.map(u=>({unitId:u.id,frames:u.frames})),contactIntervals:intervals,behaviorIntervals:behavior,events,squadrons,assets,enemyLosses:flights.reduce((n,f)=>n+f.initialAircraft-f.aircraft,0),friendlyLosses:squadrons.reduce((n,s)=>n+s.maxAircraft-s.aircraft,0),intelGained:intelReports.filter(x=>x.recovered).map(x=>x.detail),baseDamage:0,enemyBaseDamage:0,logistics:Math.min(15,state.logistics+4),command:Math.min(3,state.command+1),executionRoutes,enemyFlights:flights,defenseCues:[],defensiveAwareness:0,lessons:events.filter(e=>['DOGFIGHT ENDS','FIGHTER TRACK LOST','INTEL RECOVERED','INTEL LOST'].includes(e.title)).slice(0,4).map(e=>({title:e.title,detail:e.detail,tone:e.tone==='danger'?'danger':e.tone==='warning'?'warning':'friendly'})),playerAssets,reinforcementCalls:[],roundScore:0,baseExposure:state.baseExposure,baseExposureDelta:0,combatSequences:sequences,weaponEffects:effects,contactObservations:observations,radarTrackReceipts:receipts,interceptPlans:plans,intelReports,mappedAreas:[]};result.roundScore=battleScoreAt(result,Infinity);return result
}
