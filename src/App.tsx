...
import { applyRound, effectiveSensorRange, RADAR_RANGE, rearmSquadron, repairDefense, repairRunway, repairSquadron, resolveRound, restoreReadiness, serviceSquadronCost } from './game/engine'
import { snapToHex } from './game/hex'
import type { Asset, CombatEvent, MatchState, Point, Squadron } from './game/types'

const SAVE_KEY='dead-reckoning-mvp-v5'
...
export default function App(){
  ...
  const commit=()=>{const result=resolveRound(match);resultRef.current=result;setProgress(0);setActiveEvent(undefined);setMatch(m=>({...m,phase:'execute'}))}

  useEffect(()=>{
    if(match.phase!=='execute'||!resultRef.current)return
    const result=resultRef.current;let frame=0;let last=performance.now();const duration=18000
    const tick=(now:number)=>{const dt=(now-last)*speed;last=now;frame=requestAnimationFrame(tick);setProgress(p=>{const next=Math.min(1,p+dt/duration);const seconds=next*18;const event=[...result.events].reverse().find(e=>e.time<=seconds);setActiveEvent(event);if(next>=1){cancelAnimationFrame(frame);setTimeout(()=>setMatch(m=>applyRound(m,result)),650)}return next})}
  ...
  return <main className="app-shell">
    <div className={`game-frame phase-${match.phase}`}>
      ...
      <div className="map-wrap"><Battlefield squadrons={match.squadrons} assets={match.enemyAssets} playerAssets={match.playerAssets} selectedId={match.selectedId} placementId={placementId} phase={match.phase} progress={progress} activeEvent={activeEvent} executionResult={match.phase==='execute'?resultRef.current:match.phase==='debrief'?match.lastResult:undefined} onRoute={setRoute} onPlace={placeAsset}/><PhaseRail phase={match.phase}/><HealthBars player={match.playerBaseHealth} enemy={match.enemyBaseHealth} enemyKnown={enemyBaseKnown}/>
        ...
        {match.phase==='execute'?<ExecutionOverlay progress={progress} activeEvent={activeEvent} speed={speed} onSpeed={()=>setSpeed(s=>s===1?2:1)} match={match} result={resultRef.current} onAbort={()=>setMatch(m=>({...m,phase:'debrief'}))}/>:null}
      </div>
...
function ExecutionOverlay({progress,activeEvent,speed,onSpeed,match,result,onAbort}:{progress:number;activeEvent?:CombatEvent;speed:number;onSpeed:()=>void;match:MatchState;result?:ReturnType<typeof resolveRound>;onAbort:()=>void}){
...
function Debrief({match,onRepair,onRearm,onReady,onDefense,onRunway,onNext}:{match:MatchState;onRepair:(id:string)=>void;onRearm:(id:string)=>void;onReady:(id:string)=>void;onDefense:(id:string)=>void;onRunway:()=>void;onNext:()=>void}){
  ...
  return <section className="debrief"><div className="debrief-title"><div><small>ROUND {String(match.round).padStart(2,'0')} COMPLETE</small><h2>AFTER ACTION</h2></div><div className={`outcome ${r.enemyBaseDamage>0?'success':''}`}><CheckCircle2/><span>{r.enemyBaseDamage>0?'OBJECTIVE HIT':'INTEL GAINED'}</span></div></div>
    <div className="summary-grid"><article><small>FRIENDLY LOSSES</small><strong>{r.friendlyLosses}</strong><span>AIRCRAFT</span></article><article><small>ENEMY LOSSES</small><strong>{r.enemyLosses}</strong><span>CONFIRMED</span></article><article><small>BASE DAMAGE</small><strong>{r.enemyBaseDamage}%</strong><span>INFLICTED</span></article><article><small>DEFENSE CUE</small><strong>{r.defensiveAwareness??0}%</strong><span>AWARENESS</span></article></div>
